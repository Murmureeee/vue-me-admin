...
function generateMenus(routes: RouteRecordRaw[], router: Router): MenuRecordRaw[] {
  ...
  let menus = mapTree<ExRouteRecordRaw, MenuRecordRaw>(routes, (route: any) => {
    ...
    const { activeIcon, hideChildrenInMenu = false, icon, link, order, title = '' } = meta
  ...
  menus = menus.sort((a: any, b: any) => (a?.order ?? 999) - (b?.order ?? 999))

  // 过滤掉隐藏的菜单项
  return filterTree(menus, (menu: any) => !!menu.show)
}
